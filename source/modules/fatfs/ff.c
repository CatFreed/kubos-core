...
#include "kubos-core/modules/fatfs/ff.h"			/* Declarations of FatFs API */
#include "kubos-core/modules/fatfs/stm32f4/diskio.h"		/* Declarations of disk I/O functions */
#include <stdio.h>
	...
	for ( ;  btr;								/* Repeat until all data read */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			...
			if (cc) {							/* Read maximum contiguous sectors directly */
				...
				for (int secti = 0; secti < cc; secti++) {
					BYTE *ptr = rbuff + (SS(fp->fs) * secti);
					int sectx = sect + (SS(fp->fs) * secti);
					if (disk_read(fp->fs->drv, ptr, sectx, 1)) {
						DEBUG("disk_read sect=%lu,cc=%u failed\n", sect, cc);
			...
			if (fp->dsect != sect) {			/* Load data sector if not in cache */
				...
				if (fp->flag & FA__DIRTY) {		/* Write-back dirty sector cache */
					if (disk_write(fp->fs->drv, fp->buf, fp->dsect, 1)) {
						DEBUG("disk_write dsect=%lu failed\n", fp->dsect);
						ABORT(fp->fs, FR_DISK_ERR);
					}
					fp->flag &= ~FA__DIRTY;
				}
				...
				if (disk_read(fp->fs->drv, fp->buf, sect, 1)) {	/* Fill sector cache */
					DEBUG("disk_read sect=%lu failed\n", sect);
					ABORT(fp->fs, FR_DISK_ERR);
				}
			}
#endif
		...
		if (move_window(fp->fs, fp->dsect)) {		/* Move sector window */
			DEBUG("move_window failed, dsect=%lu\n", fp->dsect);
			ABORT(fp->fs, FR_DISK_ERR);
		}
